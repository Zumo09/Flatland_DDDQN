...
def tree_decomposition(obs):  # obs is the observation of a single agent

    tree_list = []

    tree_list.append((node_decomposition(
        obs)))  # call for the node_decomposition(obs) function (give a look below for explanation of hot it works)

    childs = childs_decomposition(obs[12],
                                  1)  # call for the child_decomposition(obs[12], 1) function: obs[12] is the
    # thirteenth attribute of a Node object;
...
def childs_decomposition(childs, depth):
    childs_list = []
    ...
    for direction in childs:  # since in 'childs' is a dict
        ...
        if childs[direction] == -np.inf:
            ...
            if depth < MAX_DEPTH:  # here, the part of filling the observation starts. This must be done in order to
                ...
                non_existing_childs = fill_childs(fill_childs_list, depth,
            ...
            node_data = node_decomposition(childs[direction])

            if childs[direction][12] != {}:  # if there are children

                childs_node_data = childs_decomposition(childs[direction][12],
...
def fill_childs(fill_childs_list, depth, letters_list):
    temp_list = []
    ...
    if depth < MAX_DEPTH:
        ...
        for letter in letters_list:  # just take a single letter
            ...
            temp_list = fill_childs(fill_childs_list, depth + 1,
...
def tree_encoding(obs):  # obs is the observation of a single agent

    tree_decomposed = tree_decomposition(obs)  # call for the tree_decomposition() function
    ...
    final_array = sub_tree_encoding(tree_decomposed[1], final_array,
...
def sub_tree_encoding(tree_decomposed, final_array, tree_dict):
    for sub_tree in tree_decomposed:
        ...
        if len(sub_tree) == 3:  # this is the base, that is when a node is not characterized by any child
            ...
            final_array = sub_tree_encoding(sub_tree[3], final_array,
