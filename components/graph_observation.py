...
from utils.graph_utils import assign_priority
...
class GraphObsForRailEnv(ObservationBuilder):
    ...
    def reset(self):
        ...
        :return:
        """
        # self.env_graph.py = map_to_graph(self.env) # Graph of the environment as tuple (nodes, edges) -
        # as computed in src.algo.graph.env_graph.py.py
        '''
    ...
    def get(self, handle: int = 0) -> {}:
        ...
        # Augment occupancy with another one-hot encoded layer: 1 if this cell is overlapping and the conflict
        # span was already entered by some other agent
        ...
        for ca in conflicting_agents:
            if ca != handle:
                ...
                ts = [x for x, y in enumerate(self.cells_sequence[handle]) if
                      y[0] == agents[ca].position[0] and y[1] == agents[ca].position[1]]  # Find index/ts for conflict
        ...
        agent_obs = np.append(agent_obs,
                              (priority, max_prio_encountered, n_agents_malfunctioning, n_agents_ready_to_depart))
    ...
    def _get_shortest_path_action(self, handle):
        ...
        if agent.status == RailAgentStatus.READY_TO_DEPART:
            ...
            # TODO Move k_shortest_paths from here - this is computationally expensive
            ...
            if shortest_paths[handle] is None:  # Railway disrupted
                ...
                '''THIS WORKS WITH NEXT VERSION next_direction = shortest_paths[handle][1].direction next_position = 
                shortest_paths[handle][1].position # COULD return None? action = get_action_for_move(agent.position, 
                agent.direction, next_position, next_direction, self.env.rail) 
    ...
    def _possible_conflict(self, handle, ts):
        """
        Function that given agent (as handle) and time step, returns a counter that represents the sum of possible
        conflicts with other agents at that time step. Possible conflict is computed considering time step (current,
        pre and stop), direction, and possibility to enter that cell in opposite direction (w.r.t. to current agent).
        Precondition: 0 <= ts <= self.max_prediction_depth - 1. Exclude READY_TO_DEPART agents from this count,
        namely, check conflicts only with agents that are already active.
        ...
        if int_pos in np.delete(self.predicted_pos[ts], handle, 0):
            ...
            for ca in conflicting_agents[0]:
                if self.env.agents[ca].status == RailAgentStatus.ACTIVE:
                    if self.predicted_dir[ts][handle] != self.predicted_dir[ts][ca] and \
                            cell_transitions[self._reverse_dir(self.predicted_dir[ts][ca])] == 1:
            ...
            for ca in conflicting_agents[0]:
                if self.env.agents[ca].status == RailAgentStatus.ACTIVE:
                    if self.predicted_dir[ts][handle] != self.predicted_dir[pre_ts][ca] and \
                            cell_transitions[self._reverse_dir(self.predicted_dir[pre_ts][ca])] == 1:
                        if not (self._is_following(ca, handle)):
                            occupancy_counter += 1
                            conflicting_agents_ts.add(ca)
            ...
            for ca in conflicting_agents[0]:
                if self.env.agents[ca].status == RailAgentStatus.ACTIVE:
                    if self.predicted_dir[ts][handle] != self.predicted_dir[post_ts][ca] and \
                            cell_transitions[self._reverse_dir(self.predicted_dir[post_ts][ca])] == 1:
                        if not (self._is_following(ca, handle)):
                            occupancy_counter += 1
                            conflicting_agents_ts.add(ca)
    ...
    def _fill_occupancy(self, handle):
        """
        Returns encoding of agent occupancy as an array where each element is 0: no other agent in this cell at this
        ts (free cell) >= 1: counter (probably) other agents here at the same ts, so conflict, e.g. if 1 => one
        possible conflict, 2 => 2 possible conflicts, etc. :param handle: agent id :return: occupancy,
        conflicting_agents
        """
    ...
    def _compute_overlapping_paths(self, handle):
        """
        Function that checks overlapping paths, where paths take into account shortest path prediction,
        so time/speed, but not the fact that the agent is moving or not. :param handle: agent id :return:
        overlapping_paths is a np.array that computes path overlapping for pairs of agents, where 1 means
        overlapping. Each layer represents overlapping with one particular agent.
        """
    ...
    def _is_following(self, handle1, handle2):
        ...
        virtual_position1 = agent1.initial_position if agent1.status == RailAgentStatus.READY_TO_DEPART \
            else agent1.position
        virtual_position2 = agent2.initial_position if agent2.status == RailAgentStatus.READY_TO_DEPART \
            else agent2.position
        ...
        if agent1.initial_position == agent2.initial_position \
                ...
                and (abs(virtual_position1[0] - virtual_position2[0]) <= 2 or
                     abs(virtual_position1[1] - virtual_position2[1]) <= 2):
            return True
        else:
            return False
