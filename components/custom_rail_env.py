...
def fast_clip(position: (int, int), min_value: (int, int), max_value: (int, int)):
    return (
...
def fast_argmax(possible_transitions: (int, int, int, int)):
        ...
        if move:
            if agent.position is None:  # agent is entering the env
                ...
                if not all([transition_valid, new_cell_valid]):
                    print(
                        f"ERRROR: step_agent2 invalid transition ag {i_agent} dir {new_direction} pos {agent.position} next {rc_next}")
