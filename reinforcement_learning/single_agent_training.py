    ...
    for episode_idx in range(n_episodes):
        ...
        for step in range(max_steps - 1):
            ...
            for agent in range(env.get_num_agents()):
                # Only update the values when we are done or when an action was taken and thus
        ...
        if episode_idx % 100 == 0:
            ...
            policy.save('./checkpoints/single-' + str(episode_idx))
            action_count = [1] * action_size
        else:
            end = " "

        print('\rTraining {} agents on {}x{}\t Episode {}\t Average Score: {:.3f}\tDones: {:.2f}%\tEpsilon: {:.2f} '
